-- Migration: Mettre à jour handle_onboarding pour gérer le rôle PROPRIETAIRE
-- Ajoute proprietaire_id dans le profil créé si l'invitation en contient un
...
BEGIN
  ...
  IF v_is_invitation AND v_invitation_token IS NOT NULL THEN
    -- Get invitation details (including proprietaire_id)
    ...
    IF FOUND THEN
      ...
      IF v_invitation.expires_at > NOW() THEN
        ...
        -- Include proprietaire_id if role is PROPRIETAIRE
        INSERT INTO public.profiles (id, organisation_id, full_name, role, proprietaire_id)
        VALUES (
          p_user_id,
          v_invitation.organisation_id,
          COALESCE(v_invitation.invited_name, p_full_name),
          v_invitation.role,
          v_invitation.proprietaire_id
        )
...
COMMENT ON FUNCTION public.handle_onboarding IS 'Creates profile for new users - handles PROPRIETAIRE role with proprietaire_id link';
