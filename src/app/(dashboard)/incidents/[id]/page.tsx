...
export default async function IncidentDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: incident } = await supabase.from("incidents").select("*, logement:logements(id, name), prestataire:prestataires(id, full_name), mission:missions(id, type)").eq("id", params.id).single();
  ...
  const mission = incident.mission as { id: string; type: string } | null;
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="flex justify-between"><span className="text-muted-foreground">Sévérité</span><StatusBadge value={incident.severity} label={INCIDENT_SEVERITY_LABELS[incident.severity as keyof typeof INCIDENT_SEVERITY_LABELS]} /></div>
          <div className="flex justify-between"><span className="text-muted-foreground">Statut</span><StatusBadge value={incident.status} label={INCIDENT_STATUS_LABELS[incident.status as keyof typeof INCIDENT_STATUS_LABELS]} /></div>
          ...
          {mission && <div className="flex justify-between"><span className="text-muted-foreground">Mission liée</span><Link href={`/missions/${mission.id}`} className="hover:underline">{mission.type}</Link></div>}
