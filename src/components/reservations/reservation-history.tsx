...
function getActionDescription(log: ActivityLog): string {
  ...
  switch (log.action) {
    ...
    case "CREATED":
      return `${actorName} a créé la réservation`;
    case "UPDATED":
      return `${actorName} a modifié la réservation`;
    case "DELETED":
      return `${actorName} a supprimé la réservation`;
    case "REMINDER_SENT":
...
export function ReservationHistory({ reservationId }: Props) {
  ...
  useEffect(() => {
    if (open && !loaded) {
      setLoading(true);
      getActivityLogs("RESERVATION", reservationId)
        .then((data) => {
          setLogs(data);
          setLoaded(true);
        })
