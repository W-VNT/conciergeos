...
import { ChevronDown, ChevronUp, ArrowRightLeft, UserPlus, MessageSquarePlus, Plus, Pencil, Trash2, Clock, Activity } from "lucide-react";
...
function getActionDescription(log: ActivityLog): string {
  ...
  switch (log.action) {
    case "STATUS_CHANGED":
      return `${actorName} a changé le statut`;
    case "ASSIGNED":
      return `${actorName} a effectué une assignation`;
    case "COMMENT_ADDED":
      return `${actorName} a ajouté un commentaire`;
    case "CREATED":
      return `${actorName} a créé le logement`;
    case "UPDATED":
      return `${actorName} a modifié le logement`;
    case "DELETED":
      return `${actorName} a supprimé le logement`;
    case "REMINDER_SENT":
...
export function LogementHistory({ logementId }: Props) {
  ...
  useEffect(() => {
    if (open && !loaded) {
      setLoading(true);
      getActivityLogs("LOGEMENT", logementId)
        .then((data) => {
          setLogs(data);
          setLoaded(true);
        })
        .catch((err) => {
          console.error("Failed to load logement history:", err);
        })
        .finally(() => setLoading(false));
    }
  }, [open, loaded, logementId]);
